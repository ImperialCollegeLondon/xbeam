...
!-> Private Variables.
!
  real(8),private, parameter :: pi=4.d0*atan(1.d0)
...
! Compute original norm.
  OldNormPsi=sqrt(dot_product(Psi,Psi))
...
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
 function rotvect_theta2psi (Theta) result(Psi)

! I/O Variables.
  real(8),intent(in)  :: Theta(:)       ! Rodrigues Parameters.
...
! Compute rotation vector.
  Psi= 2.d0*atan(NormTheta*0.5d0)*Normal
...
! Transform rotation vector to quaternions.
  rotvect_psi2quat(1)  = cos(NormPsi*0.5d0)

! Small angles approximation.
  if (abs(NormPsi).lt.rot_Epsilon) then
    rotvect_psi2quat(2:4)=(0.5d0-NormPsi*NormPsi/12.d0)*Psi
! Regular expression.
  else
...
! Larger angles.
    ...
    RotMatrix= Unit + sin(NormPsi)*SkewNormal               &
&            + (1.d0 - cos(NormPsi))*matmul(SkewNormal,SkewNormal)
  end if
...
! Small angles.
  if (abs(NormPsi).le.rot_Epsilon) then
    RotInt= Unit + 0.5d0*SkewPsi + (1.d0/6.d0)*matmul(SkewPsi,SkewPsi)

! Larger angles.
  else
    RotInt= Unit + ((1.d0-cos(NormPsi))/(NormPsi*NormPsi))*SkewPsi     &
&                + ((NormPsi-sin(NormPsi))/(NormPsi*NormPsi*NormPsi))  &
...
! For larger angles.
    ...
    K1= (1.d0 - cos(NormPsi))/(NormPsi*NormPsi)
    K2= sin(NormPsi)/NormPsi
