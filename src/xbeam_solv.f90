...
subroutine xbeam_solv_couplednlndyn_step_updated(&
  ...
  use iso_c_binding
    ...
    ! real(c_double), intent(INOUT)                   :: dqdt()
...
! Local variables.
  ...
  real(8)                           :: dXdt(numdof),dXddt(numdof)   ! Generalized coordinates and derivatives for structure.
  real(8)                           :: X(numdof)
  real(8)                           :: dQddt(numdof + 10)   ! Generalized coordinates and derivatives for coupled system.
  real(8)                           :: Q(numdof + 10)
  real(8)                           :: dQdt(numdof + 10)
  real(8)                           :: DQ(numdof + 10)
  ...
  ! Define vaiables for output information.
  character(len=80)                 :: Text          ! Text with current time information to print out.
  real(8)                           :: Displ(n_node, 6)    ! Current nodal displacements/rotations.
  real(8)                           :: Veloc(n_node, 6)    ! Current nodal velocities.
 ...
 ! variables for error based check
  ...
  real(8)       :: initialQ(numdof + 10)
  real(8)       :: initialdQdt(numdof + 10)
  ...
  !---------------------------
    ...
    ! dQddt(NumDof+1:NumDof+6) = for_acc
    ...
    ! ACoa(4:6,4:6) = transpose(Cao)
        ...
        do Iter=1,Options%MaxIterations+1
            ...
            !print*, 'Line 739, xbeam solv'
            !print*, size(Elem)
            call cbeam3_solv_state2disp (Elem,Node,pos_ini,psi_ini,X,dXdt,pos_def,psi_def,pos_dot_def,psi_dot_def)
            ...
            call cbeam3_asbly_dynamic (numdof, n_node, n_elem, Elem,Node,pos_ini,psi_ini,pos_def,&
            psi_def,pos_dot_def,psi_dot_def,0.d0*pos_def,0.d0*psi_def,&
            static_forces+dynamic_forces,dQdt(NumDof+1:NumDof+6),&
            0.0d0*dQddt(NumDof+1:NumDof+6),MSS,MSR,CSS,CSR,&
            KSS,Felast,Qelast,Options,Cao)

            call xbeam_asbly_dynamic (numdof, n_node, n_elem, Elem,Node,pos_ini,psi_ini,pos_def,psi_def,&
            pos_dot_def,psi_dot_def,0.d0*pos_def,0.d0*psi_def,&
            dQdt(NumDof+1:NumDof+6),0.0d0*dQddt(NumDof+1:NumDof+6),&
            dQdt(NumDof+7:NumDof+10),MRS,MRR,CRS,CRR,CQR,CQQ,&
            KRS,Frigid,Qrigid,Options,Cao)
            ...
            Qelast = Qelast - matmul(Felast,fem_m2v(static_forces+dynamic_forces,NumDof,Filter=ListIN))
            Qrigid = Qrigid - matmul(Frigid,fem_m2v(static_forces+dynamic_forces,NumDof+6))

            Qtotal(1:NumDof)          = Qelast
            Qtotal(NumDof+1:NumDof+6) = Qrigid
            Qtotal(NumDof+7:NumDof+10)= matmul(CQQ,dQdt(NumDof+7:NumDof+10))

            ! print*, maxval(abs(MSR - transpose(MRS)))
            ...
            ! Asys = 0.0d0
            ! call mat_addmat(0,0,Ktotal,Asys)
            ! call mat_addmat(0,0,Ctotal*gamma/(beta*dt),Asys)
            ! call mat_addmat(0,0,Mtotal*1.d0/(beta*dt*dt),Asys)
            Asys = Ktotal
            Asys = Asys + Ctotal*gamma/(beta*dt)
            Asys = Asys + Mtotal*1.d0/(beta*dt*dt)
        ...
        ! Export velocities and accelerations in inertial frame
            ...
            ACoa(1:3,1:3) = transpose(Cao)
            ACoa(4:6,4:6) = transpose(Cao)
            for_vel = matmul(ACoa,dQdt (NumDof+1:NumDof+6))
            for_acc = matmul(ACoa,dQddt(NumDof+1:NumDof+6))
        end if
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!-> Subroutine XBEAM_SOLV_COUPLEDNLNDYN_STEP
!
!-> Description:
!
!    Nonlinear dynamic solution of multibeam problem under applied forces,
...
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
 subroutine xbeam_solv_couplednlndyn_step (iOut,&
                                           NumDof,&
                                           Time,&
                                           Elem,&
                                           Node,&
...
return
end subroutine xbeam_solv_couplednlndyn_step
! !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! !-> Subroutine XBEAM_SOLV_COUPLEDNLNDYN_STEP
! !
! !-> Description:
...
!   real(8),      intent(in)   :: F0        (:,:)   ! Applied static nodal forces.
!   real(8),      intent(in)   :: Fdyn      (:,:)! Dynamic nodal forces.
!   real(8),      intent(in)   :: previous_Fdyn(:,:)! Dynamic nodal forces.
!   real(8),      intent(inout):: quat    (:)
!   real(8),      intent(inout)  :: Vrel      (:,:)   ! Time history of the velocities of the reference frame.
!   real(8),      intent(inout)  :: VrelDot   (:,:)   ! Time history of the accelerations of the reference frame.
!   real(8),      intent(in)   :: Coords    (:,:)   ! Initial coordinates of the grid points.
!   real(8),      intent(in)   :: Psi0      (:,:,:) ! Initial CRV of the nodes in the elements.
...
!     Fsc = 1.0_8; Msc = 1.0_8;
!     do iStep=1,size(Time)-1
!         Fsc = max( maxval(abs( F0(:,1:3)+Fdyn(:,1:3) )), Fsc);
!         Msc = max( maxval(abs( F0(:,4:6)+Fdyn(:,4:6) )), Msc);
!     end do
!
!     ! Initialise
!     NumN=size(Node)
...
!     ! sm: why we are neglecting it? Let's set it to be on-zero
!     ! call cbeam3_asbly_dynamic (Elem,Node,Coords,Psi0,PosDefor,PsiDefor,PosDotDefor,PsiDotDefor,0.d0*PosDefor,0.d0*PsiDefor,   &
!     ! &                            F0+previous_Fdyn,dQdt(NumDof+1:NumDof+6),            &
!     ! &                            0.d0*dQddt(NumDof+1:NumDof+6),                     &
!     ! &                            ms,MSS,MSR,cs,CSS,CSR,ks,KSS,fs,Felast,Qelast,Options,Cao)
!     ! Qelast= Qelast - sparse_matvmul(fs,Felast,NumDof,fem_m2v(F0+Fdyn,NumDof,Filter=ListIN))
!     call cbeam3_asbly_dynamic (Elem,Node,Coords,Psi0,PosDefor,PsiDefor,PosDotDefor,PsiDotDefor,0.d0*PosDefor,0.d0*PsiDefor,   &
!     &                            F0+previous_Fdyn,dQdt(NumDof+1:NumDof+6),            &
!     &                            dQddt(NumDof+1:NumDof+6),                     &
!     &                            ms,MSS,MSR,cs,CSS,CSR,ks,KSS,fs,Felast,Qelast,Options,Cao)
!     Qelast= Qelast - sparse_matvmul(fs,Felast,NumDof,fem_m2v(F0+previous_Fdyn,NumDof,Filter=ListIN))
!     !
!     ! call xbeam_asbly_dynamic (Elem,Node,Coords,Psi0,PosDefor,PsiDefor,PosDotDefor,PsiDotDefor,0.d0*PosDefor,0.d0*PsiDefor,    &
!     ! &                           dQdt(NumDof+1:NumDof+6),                            &
!     ! &                           0.d0*dQddt(NumDof+1:NumDof+6),                      &
...
!         Qrigid(1:3)=0.d0
!     end if
!     Qrigid= Qrigid - sparse_matvmul(fr,Frigid,6,fem_m2v(F0+Fdyn,NumDof+6))
!
!     ! Assemble coupled system
!     Qtotal(1:NumDof)          = Qelast
!     Qtotal(NumDof+1:NumDof+6) = Qrigid
...
!
!         ! corrector
!         Q = Q + beta*dQddt*dt*dt
!         dQdt = dQdt + gamma*dQddt*dt
!
!         ! Spherical Joint - sm
!         ! Predictor step is forced to be zero for translational dofs.
!         if (SphFlag) then
...
!             ! call cbeam3_asbly_dynamic (Elem,Node,Coords,Psi0,PosDefor,&
!             ! PsiDefor,PosDotDefor,PsiDotDefor,0.d0*PosDefor,0.d0*PsiDefor,&
!             ! F0+Fdyn(:,:),dQdt(NumDof+1:NumDof+6),&
!             ! 0.d0*dQddt(NumDof+1:NumDof+6), ms,MSS,MSR,cs,CSS,CSR,ks,&
!             ! KSS,fs,Felast,Qelast,Options,Cao)
!             !
!             ! call xbeam_asbly_dynamic (Elem,Node,Coords,Psi0,PosDefor,PsiDefor,&
...
!             PsiDefor,PosDotDefor,PsiDotDefor,0.d0*PosDefor,0.d0*PsiDefor,&
!             F0+Fdyn(:,:),dQdt(NumDof+1:NumDof+6),&
!             dQddt(NumDof+1:NumDof+6), ms,MSS,MSR,cs,CSS,CSR,ks,&
!             KSS,fs,Felast,Qelast,Options,Cao)
!
!             call xbeam_asbly_dynamic (Elem,Node,Coords,Psi0,PosDefor,PsiDefor,&
!             PosDotDefor,PsiDotDefor,0.d0*PosDefor,0.d0*PsiDefor,&
!             dQdt(NumDof+1:NumDof+6),dQddt(NumDof+1:NumDof+6),&
!             dQdt(NumDof+7:NumDof+10),mr,MRS,MRR,cr,CRS,CRR,CQR,CQQ,kr,&
!             KRS,fr,Frigid,Qrigid,Options,Cao)
!
!             ! Spherical Joint - sm:
...
!         ! Export velocities and accelerations in body frame
!         if (Options%OutInaframe) then
!             Vrel(2, :)    = dQdt (NumDof+1:NumDof+6)
!             VrelDot(2, :) = dQddt(NumDof+1:NumDof+6)
!         ! Export velocities and accelerations in inertial frame
!         else
!             Cao = xbeam_Rot(quat)
!             ACoa(1:3,1:3) = transpose(Cao)
!             ACoa(4:6,4:6) = transpose(Cao)
!             Vrel(2,:)    = matmul(ACoa,dQdt (NumDof+1:NumDof+6))
!             VrelDot(2,:) = matmul(ACoa,dQddt(NumDof+1:NumDof+6))
!         end if
!
!     ! end do
!
...
! end subroutine xbeam_solv_couplednlndyn_step
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!-> Subroutine XBEAM_SOLV_COUPLEDNLNDYN_OLD
!
!-> Description:
!
!    Nonlinear dynamic solution of multibeam problem under applied forces,
...
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!  subroutine xbeam_solv_couplednlndyn_old (iOut,NumDof,Time,Elem,Node,F0,Fa,Ftime,           &
! &                                     Vrel,VrelDot,Quat,Coords,Psi0,PosDefor,PsiDefor,  &
...
!
!   return
!  end subroutine xbeam_solv_couplednlndyn_old
!
!
!
! !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
